...
@Component({
  selector: 'app-detalles-mantenimiento-tecnico',
  imports: [CommonModule, FormsModule, SidebarTecnico],
  templateUrl: './detalles-mantenimiento-tecnico.html',
  styleUrl: './detalles-mantenimiento-tecnico.scss',
})
export class DetallesMantenimientoTecnico {
  // estado
  cargando = false;
    ...
    this.mantenimientoService.eliminar(this.id_mantenimiento).subscribe({
      next: () => {
        ...
        this.router.navigate(
          id_maquina
            ? ['/detalles-maquinaria-tecnico', id_maquina]
            : ['/ver-mantenimientos-tecnico']
        );
      },
      error: () => {
        this.cargandoEliminacion = false;
