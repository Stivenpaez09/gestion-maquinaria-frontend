...
@Component({
  selector: 'app-detalles-registro-horas-maquinaria-operador',
  imports: [CommonModule, FormsModule, RouterLink, SidebarOperador],
  templateUrl: './detalles-registro-horas-maquinaria-operador.html',
  styleUrl: './detalles-registro-horas-maquinaria-operador.scss',
})
export class DetallesRegistroHorasMaquinariaOperador {
  ...
  eliminarRegistro(): void {
    ...
    this.registroService.eliminar(this.id_registro).subscribe({
      next: () => {
        ...
        this.router.navigate(['/ver-registros-horas-maquinaria-operador']);
      },
      error: (err) => {
        this.cargandoEliminacion = false;
  ...
  irADetallesMaquinaria(id: number): void {
    this.router.navigate(['/detalles-maquinaria-operador', id]);
  }
}
