...
@Component({
  selector: 'app-detalles-mantenimiento-programado-admin',
  imports: [CommonModule, FormsModule, RouterModule, RouterLink, SidebarAdmin],
  templateUrl: './detalles-mantenimiento-programado-admin.html',
  styleUrl: './detalles-mantenimiento-programado-admin.scss',
})
export class DetallesMantenimientoProgramadoAdmin implements OnInit {
  cargando = false;
  ...
  eliminarMantenimiento(): void {
    ...
    this.mantenimientoService.eliminar(this.id_programado).subscribe({
      next: () => {
        ...
        if (id_maquina) {
          this.router.navigate(['/detalles-maquinaria-admin', id_maquina]);
        } else {
